# ---- import models ---------------
from models.LITEWAYlight import LITEWAYLight
from models.LITEWAYfull import LITEWAY
from models.Attend import AttendDiscriminate
from models.SA_HAR import SA_HAR
from models.deepconvlstm import DeepConvLSTM
from models.TinierHAR import TinierHAR_Model
...
class model_builder(nn.Module):
    ...
    def __init__(self, args, input_f_channel = None):
        ...
        if self.args.model_type == "liteway":
            ...
            self.model = LITEWAY((1, f_in, self.args.input_length, self.args.c_in), self.args.num_classes, config)
            print(f"Build the {self.args.model_type} model!")

        elif self.args.model_type == "liteway_light":
            config_file = open('configs/model.yaml', mode='r')
            config = yaml.load(config_file, Loader=yaml.FullLoader)[self.args.model_type]
            self.model = LITEWAYLight((1, f_in, self.args.input_length, self.args.c_in), self.args.num_classes, config)
            print(f"Build the {self.args.model_type} model!")
            ...
            self.model  = MLP((1,f_in, self.args.input_length, self.args.c_in ), 
                                self.args.num_classes,
                                config["filter_scaling_factor"],
                                config)
            ...
            print(config)
            self.model  = TinierHAR_Model((1,f_in, self.args.input_length, self.args.c_in ), 
                                       self.args.num_classes,
                                       config["filter_scaling_factor"],
                                       config)
            ...
            print(config)
            self.model  = LSTM_Attention((1,f_in, self.args.input_length, self.args.c_in ), 
                                       self.args.num_classes,
                                       config)
            print("Build the LSTM_Attention model!")
        elif self.args.model_type == "conv1d_lowranklstm":
            config_file = open('configs/model.yaml', mode='r')
            config = yaml.load(config_file, Loader=yaml.FullLoader)["conv1d_lowranklstm"]
            print(config)
            self.model  = Conv1D_LowRankLSTM((1,f_in, self.args.input_length, self.args.c_in ), 
                                       self.args.num_classes,
                                       config["filter_scaling_factor"],
                                       config)
            print("Build the Conv1D_LowRankLSTM model!")
            ...
            print(config)
            self.model  = Conv1D_LowRankLSTM_Attention((1,f_in, self.args.input_length, self.args.c_in ), 
                                       self.args.num_classes,
                                       config["filter_scaling_factor"],
                                       config)
            print("Build the Conv1D_LowRankLSTM_Attention model!")
            ...
            print(config)
            self.model  = Conv1D_LowRankLSTM_Self_Attention((1,f_in, self.args.input_length, self.args.c_in ), 
                                       self.args.num_classes,
                                       config["filter_scaling_factor"],
                                       config)
            print("Build the Conv1D_LowRankLSTM_Self_Attention model!")
        elif self.args.model_type == "self_attention_har":
            config_file = open('configs/model.yaml', mode='r')
            config = yaml.load(config_file, Loader=yaml.FullLoader)["self_attention_har"]
            print(config)
            self.model  = SelfAttentionHAR((1,f_in, self.args.input_length, self.args.c_in ), 
                                       self.args.num_classes,
                                       config)
