...
class Exp(object):
    def __init__(self, args):
        ...
        self.optimizer_dict = {"Adam":optim.AdamW}
    ...
    def get_setting_name(self):
        if self.args.model_type == "liteway":
            config_file = open('configs/model.yaml', mode='r')
            config = yaml.load(config_file, Loader=yaml.FullLoader)[self.args.model_type]
            setting = f"{self.args.model_type}_data_{self.args.data_name}_seed_{self.args.seed}_windowsize_{self.args.windowsize}_conv_{config['nb_conv_blocks']}_filter_{config['nb_filters']}"
            return setting
        elif self.args.model_type == "liteway_light":
            config_file = open('configs/model.yaml', mode='r')
            config = yaml.load(config_file, Loader=yaml.FullLoader)[self.args.model_type]
            setting = f"{self.args.model_type}_data_{self.args.data_name}_seed_{self.args.seed}_windowsize_{self.args.windowsize}_conv_{config['nb_conv_blocks']}_filter_{config['nb_filters']}"
            return setting
            ...
            return setting
        elif self.args.model_type == "tinierhar":
            config_file = open('configs/model.yaml', mode='r')
            config = yaml.load(config_file, Loader=yaml.FullLoader)["tinierhar"]
            setting = "tinierhar_data_{}_seed_{}_windowsize_{}_cvfilter_{}_grufilter_{}".format(self.args.data_name,
                                                                                                self.args.seed,
                                                                                                self.args.windowsize,
                                                                                                config["nb_filters"],
                                                                                                config["nb_units_gru"]
            ...
            config_file = open('configs/model.yaml', mode='r')
            config = yaml.load(config_file, Loader=yaml.FullLoader)["lstm_attention"]
            setting = "lstm_attention_data_{}_seed_{}_windowsize_{}".format(self.args.data_name,
                                                                                                                                                        self.args.seed,
                                                                                                                                                        self.args.windowsize
                                                                                                                                                        )
            return setting
            ...
            config_file = open('configs/model.yaml', mode='r')
            config = yaml.load(config_file, Loader=yaml.FullLoader)["mlp"]
            setting = "mlp_data_{}_seed_{}_windowsize_{}_FCL1_{}_FCL2_{}_FCL3_{}".format(self.args.data_name,
                                                                                                        self.args.seed,
                                                                                                        self.args.windowsize,
                                                                                                        config["numNueronsFCL1"],
                                                                                                        config["numNueronsFCL2"],
                                                                                                        config["numNueronsFCL3"])
                                                                                                    
            return setting
        elif self.args.model_type == "conv1d_lowranklstm":
            config_file = open('configs/model.yaml', mode='r')
            config = yaml.load(config_file, Loader=yaml.FullLoader)["conv1d_lowranklstm"]
            setting = "conv1d_lowranklstm_data_{}_seed_{}_windowsize_{}_rank_{}_nb_layers_lstm_{}".format(self.args.data_name,
                                                                                                        self.args.seed,
                                                                                                        self.args.windowsize,
                                                                                                        config["rank"],
                                                                                                        config["nb_layers_lstm"])
                                                                                                    
            return setting
        elif self.args.model_type == "conv1d_lowranklstm_attention":
            config_file = open('configs/model.yaml', mode='r')
            config = yaml.load(config_file, Loader=yaml.FullLoader)["conv1d_lowranklstm_attention"]
            setting = "conv1d_lowranklstm_attention_data_{}_seed_{}_windowsize_{}_rank_{}_nb_layers_lstm_{}".format(self.args.data_name,
                                                                                                        self.args.seed,
                                                                                                        self.args.windowsize,
                                                                                                        config["rank"],
                                                                                                        config["nb_layers_lstm"])
            ...
            return setting
        elif self.args.model_type == "conv1d_lowranklstm_self_attention":
            config_file = open('configs/model.yaml', mode='r')
            config = yaml.load(config_file, Loader=yaml.FullLoader)["conv1d_lowranklstm_self_attention"]
            setting = "conv1d_lowranklstm_self_attention_data_{}_seed_{}_windowsize_{}_rank_{}_nb_layers_lstm_{}".format(self.args.data_name,
                                                                                                        self.args.seed,
                                                                                                        self.args.windowsize,
                                                                                                        config["rank"],
                                                                                                        config["nb_layers_lstm"])
            ...
            return setting
        elif self.args.model_type == "self_attention_har":
            config_file = open('configs/model.yaml', mode='r')
            config = yaml.load(config_file, Loader=yaml.FullLoader)["self_attention_har"]
            setting = "self_attention_har_data_{}_seed_{}_windowsize_{}_rank_{}".format(self.args.data_name,
                                                                                                        self.args.seed,
                                                                                                        self.args.windowsize,
                                                                                                        config["rank"])
    ...
    def train(self):
        ...
        print("================ dataset: {} ====================".format(self.args.data_name))
            ...
            if skip_train:
                ...
                for epoch in range(self.args.train_epochs):
                    ...
                    for i, (batch_x1,batch_x2,batch_y) in enumerate(train_loader):
                        ...
                        batch_x1_to_save = batch_x1.cpu().numpy()
                        np.save(cv_path + "/" + 'batch_x.npy', batch_x1_to_save)
                        batch_y_to_save = batch_y.cpu().numpy()
                        np.save(cv_path + "/" + 'batch_y.npy', batch_y_to_save)
                ...
                score_log.write("Test Accuracy: {0:.7f}  Test weighted F1: {1:.7f}  Test macro F1 {2:.7f}\n".format(test_acc, test_f_w, test_f_macro))
                score_log.flush()

                test_acc_all_cv.append(test_acc)
                test_f_w_all_cv.append(test_f_w)
                test_f_macro_all_cv.append(test_f_macro)
                if iter == (num_of_cv -1) :
                    score_log.write("Averaged Test Accuracy: {0:.7f}  Averaged Test weighted F1: {1:.7f}  Averaged Test macro F1 {2:.7f}\n".format(np.average(test_acc_all_cv), np.average(test_f_w_all_cv), np.average(test_f_macro_all_cv)))
                    score_log.flush()
